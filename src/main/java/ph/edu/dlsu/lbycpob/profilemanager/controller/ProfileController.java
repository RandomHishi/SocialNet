...
public class ProfileController {
    ...
    public List<Dtos.ProfileListItem> listProfiles() {
        return
                profileService.listProfiles().stream().map(ProfileListItem::of).toList();
    }
    @GetMapping("/{id}")
    ...
    @GetMapping("/lookup")
    public ProfileDetail lookupProfile(@RequestParam String query) {Profile profile = profileService.lookupFirstMatch(query);
        return ProfileDetail.of(profile,
    ...
    public Map<String, String> updateStatus(@PathVariable UUID id, @RequestBody
    UpdateStatusRequest request) {
        profileService.updateStatus(id, request.status());
        return Map.of("status", request.status().trim());
    }@PatchMapping("/{id}/quote")
    public Map<String, String> updateQuote(@PathVariable UUID id, @RequestBody
    ...
    public Map<String, String> updatePictureUrl(@PathVariable UUID id, @RequestBody
    ...
    public PictureResult uploadAvatar(@PathVariable UUID id, @RequestParam("file")
    MultipartFile file){
        String url = profileService.updatePictureFromUpload(id, file);
        return new PictureResult(url);
    }
    @PostMapping("/{id}/friends")
    public Map<String, String> addFriend(@PathVariable UUID id, @RequestBody
    ...
    public Map<String, String> removeFriend(@PathVariable UUID id, @RequestBody
    FriendActionRequest request) {
