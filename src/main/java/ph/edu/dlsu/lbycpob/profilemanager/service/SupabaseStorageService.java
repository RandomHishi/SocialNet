 ...
 * REST endpoint (Storage API reference: {@code POST
 ...
 * SDK, no reverse engineering required. See: <a
 href="https://supabase.com/docs/guides/storage">...</a>
 * <p>
...
public class SupabaseStorageService {
     ...
     */
    public String uploadAndGetPublicUrl(String path, byte[] content, String
            contentType) {
        if (secretKey == null || secretKey.isBlank()) {
            throw new IllegalStateException(
                    "SUPABASE_SECRET_KEY is not configured -- avatar upload is
                    unavailable. " +
                    "Set it to your Supabase Secret API key (sb_secret_...)
                    from " +
                    "Project Settings -> API Keys -> Publishable and secret
                    API keys, " +
                    "or use the 'paste an image URL' field instead.");
        }
        URI uploadUri = URI.create(supabaseUrl + "/storage/v1/object/" + bucket + "/"
                + path);
        HttpRequest request = HttpRequest.newBuilder(uploadUri)
                .header("apikey", secretKey)
                .header("Authorization", "Bearer " + secretKey)
                .header("Content-Type", contentType)
                .header("x-upsert", "true") // overwrite any existing file at this
        path
        ...
        } catch (Exception e) {throw new IllegalStateException("Could not reach Supabase Storage: " +
                e.getMessage(), e);
        }
        if (response.statusCode() < 200 || response.statusCode() >= 300) {
            throw new IllegalStateException(
                    "Supabase Storage upload failed (HTTP " + response.statusCode() +
                            "): " + response.body());
        }
